...
- (void)setServerUrl:(NSString *)serverUrl {
    ...
    if([serverUrl rangeOfString:@"/events.svc"].length!=0){
        ...
        _serverURL = [_serverURL stringByAppendingFormat:@"%@dcsverbose=%@",link,d];
...
- (void)_flush:(BOOL) vacuumAfterFlushing {
    ...
    BOOL (^flushByPost2Sdc)(NSArray *, NSString *) = ^(NSArray *recordArray, NSString *type) {
        ...
        for(int i=0;i<recordArray.count;i++){
            @try {
                ...
                //ycf-start
                @try{
                    if(![[dict objectForKey:@"properties"] objectForKey:@"dl"]){
                        if([ev isEqualToString:@"$AppViewScreen"] || [ev isEqualToString:@"$pageview"]){
                            [dict setValue:@"0" forKey:@"WT.dl"];
                        }else if([ev isEqualToString:@"$AppClick"] || [ev isEqualToString:@"$WebClick"]){
                            [dict setValue:@"25" forKey:@"WT.dl"];
                        }else if([ev isEqualToString:@"AppCrashed"]){
                            [dict setValue:@"99" forKey:@"WT.dl"];
                            if([[dict objectForKey:@"properties"] objectForKey:@"app_crashed_reason"]){
                                NSString * mReason = [[dict objectForKey:@"properties"] objectForKey:@"app_crashed_reason"];
                                mReason = [mReason stringByReplacingOccurrencesOfString:@"  " withString:@""];                                                    
                                mReason = [mReason stringByReplacingOccurrencesOfString:@"\t" withString:@""];
                                mReason = [mReason stringByReplacingOccurrencesOfString:@"\n" withString:@""];
                                mReason = [mReason stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
                                [[dict objectForKey:@"properties"] setValue:mReason forKey:@"app_crashed_reason"];
                            }
                        }else if([ev isEqualToString:@"$AppStart"]){
                            [dict setValue:@"22" forKey:@"WT.dl"];
                        }else if([ev isEqualToString:@"$AppEnd"]){
                            [dict setValue:@"23" forKey:@"WT.dl"];
                        }else{
                            [dict setValue:@"21" forKey:@"WT.dl"];
                        }
                    }else{
                        [dict setValue:[[dict objectForKey:@"properties"] objectForKey:@"dl"] forKey:@"WT.dl"];
                    }
                }@catch(NSException *e){
                    SAError(@"%@ dl error: %@", self, e);
                }
                //ycf-end
                ...
                @try{
                    if([dict objectForKey:@"distinct_id"]){
                        [dict setValue:[dict objectForKey:@"distinct_id"] forKey:@"WT.co_f"];
                    }
                    if([[dict objectForKey:@"properties"] objectForKey:@"$title"]){
                        [dict setValue:[[dict objectForKey:@"properties"] objectForKey:@"$title"] forKey:@"WT.ti"];
                    }
                }@catch(NSException *e){
                     SAError(@"%@ WT.co_f error: %@", self, e);
                }
    ...
    //            SALog(postBody);
           ...
           @try{
                 ...
                 SAError(@"%@ userAgent error: %@", self, e);
...
- (NSDictionary *)collectAutomaticProperties {
    ...
    [p addEntriesFromDictionary:@{
                                  ...
                                  SA_EVENT_COMMON_PROPERTY_SCREEN_HEIGHT: @((NSInteger)(size.height * scale_screen)),
                                  SA_EVENT_COMMON_PROPERTY_SCREEN_WIDTH: @((NSInteger)(size.width * scale_screen)),
                                      }];
    //ycf-start新增分辨率组合
    @try{
        
        NSNumber * mScreen_height = @((NSInteger)(size.height * scale_screen));
        NSNumber * mScreen_width = @((NSInteger)(size.width * scale_screen));
        NSString * mScreen_sr = nil;
        if(mScreen_width && mScreen_height){
            mScreen_sr = [NSString stringWithFormat:@"%@x%@",mScreen_height,mScreen_width];
        }
        [p setValue:mScreen_sr forKey:@"$screen_sr"];
    }@catch(NSException *e){
        SAError(@"%@ error: %@", self, e);
    }
    //ycf-end
